...
interface ResizableContextType {
  ...
  toggleResizable: (saveId: string, state?: "expanded" | "collapsed") => void;
  getResizableState: (saveId: string) => "expanded" | "collapsed";
...
export const ResizableProvider: React.FC<ResizableProviderProps> = ({ children }) => {
  ...
  const toggleResizable = (saveId: string, state?: "expanded" | "collapsed") => {
      ...
      const newResizablePanels = prev.find((panel) => panel.saveId === saveId)
        ? prev.map((panel) =>
            panel.saveId === saveId
              ? {
                  saveId: saveId,
                  state: state ? state : panel.state === "expanded" ? "collapsed" as const : "expanded" as const,
                }
              : panel,
          )
        : [
            ...prev,
            { saveId: saveId, state: "expanded" as const },
          ];
      
      // Save to localStorage with the new state
      localStorage.setItem("resizablePanels", JSON.stringify(newResizablePanels));
      return newResizablePanels;
    });
  };

  const getResizableState = (saveId: string): "expanded" | "collapsed" => {
    const panel = resizablePanels.find((panel) => panel.saveId === saveId);
    return panel ? panel.state : "expanded"; // Default to expanded
  };
