...
/** SQLite dialect: identifiers in double quotes, `?` placeholders. */
export const sqliteAdapter: SqlAdapter = {
  kind: "sqlite",
  quoteIdent,
    ...
    switch (details.kind) {
      ...
      case "insert": {
        ...
        if (provided.length === 0) {
          return {
            sql: `INSERT INTO ${quoteIdent(details.table)} DEFAULT VALUES`,
            params: [],
          };
        }
      ...
      case "insert_literal": {
        ...
        const vals = details.columns
          .map((c, i) =>
            toLiteral(details.values[i] ?? null, details.types?.[c]),
          )
          .join(", ");
        return {
