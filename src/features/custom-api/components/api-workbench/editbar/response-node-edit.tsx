...
import { ResponseNodeData, EndpointNodeData } from "@/features/custom-api/types/custom-api.type";
import JsonEditor, { JsonEditorRef } from "./json-editor";

// Validation schema
const responseNodeSchema = z.object({
  statusCode: z.number().min(100).max(599, "Status code must be between 100 and 599"),
  responseBody: z.string().min(1, "Response body is required").refine(
    (val) => {
      try {
...
const ResponseNodeEdit = () => {
  ...
  const getParentNodeData = () => {
    ...
    const incomingEdge = currentEndpointState.edges.find(edge => edge.target === selectedNodeId);
    if (!incomingEdge) return null;
    
    // Find the parent node
    const parentNode = currentEndpointState.nodes.find(node => node.id === incomingEdge.source);
    return parentNode;
  };
  ...
  const getAvailableFields = () => {
    ...
    const endpoint = parentNode.data.endpoint;
  ...
  const insertField = (fieldValue: string) => {
    ...
    setValue('responseBody', newValue, { shouldDirty: true, shouldValidate: true });
  ...
  useEffect(() => {
    if (selectedNodeId && currentEndpointState) {
      const node = currentEndpointState.nodes.find(n => n.id === selectedNodeId);
  ...
  return (
    <div className="pt-4">
      <h4 className="mb-3 text-sm font-medium text-foreground">Edit Response Node</h4>
        ...
        <div className="space-y-2">
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              onCheckedChange={(checked) => setUseCustomStatusCode(checked === true)}
            />
            <Label htmlFor="use-custom-status" className="text-xs font-medium text-foreground">
          ...
          {errors.statusCode && (
            <p className="text-xs text-destructive">{errors.statusCode.message}</p>
          )}
        </div>

        {/* Response Body */}
        <div className="space-y-2">
          <Label htmlFor="response-body" className="text-xs font-medium text-foreground">
            Response Body
          </Label>
